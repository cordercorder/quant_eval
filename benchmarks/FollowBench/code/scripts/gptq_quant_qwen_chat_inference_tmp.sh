...
CUDA_VISIBLE_DEVICES=0,1 nohup python -u code/gptq_quant_qwen_chat_inference.py \
    --model_name gptq_${bits_map[${num_bits}]}_Qwen-${model_size}-Chat \
    --checkpoint_path ${checkpoint_path} \
    --quant_checkpoint_path ${quant_checkpoint_path} \
    --constraint_types example mixed >> code/scripts/logs/gptq_quant_qwen_chat_inference.gptq_${bits_map[${num_bits}]}_Qwen-${model_size}-Chat.logs 2>&1 &
