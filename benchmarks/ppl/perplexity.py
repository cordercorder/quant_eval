...
class Perplexity(evaluate.Metric):
    ...
    def _compute(
        self, predictions, model:AutoModelForCausalLM, tokenizer:AutoTokenizer, batch_size: int = 16, add_start_token: bool = True, device=None, max_length=None, encodings = None
    ):
        ...
        if not encodings:
            ...
            attn_masks= torch.zeros(len(encodings), max_len, dtype=torch.long)
            for idx, encode in enumerate(encodings):
                input_ids[idx, :encode["input_ids"].shape[-1]] = encode["input_ids"]
                input_ids[idx, encode["input_ids"].shape[-1]:] = tokenizer.pad_token_id
                attn_masks[idx, :encode["input_ids"].shape[-1]] = encode["attention_mask"]
            encodings = {
                "input_ids": input_ids,
                "attention_mask": attn_masks
            }
            encodings = BatchEncoding(encodings, tensor_type='pt')
