...
# Modify this to adapt to your case
this.dir <- "/mnt/d/codes/quant_eval/benchmarks/bbq"
setwd(this.dir)
...
# --------- PREPARE DATA -------------
uqa_files <- list.files("./results",full.names = T, pattern = "*.jsonl")
dat_uqa <- NULL
for(f in uqa_files){
  temp = read_json(f)
...
write.csv(dat_bias, file="debug_bbq.csv")
# plotting
