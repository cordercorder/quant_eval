...
if [[ `hostname` = "tj5-g8-a100-v6-tj5-translator03.kscn" ]]; then
    ...
    save_result_path=/home/work/jinrenren/quant_eval_results/memory/${dataset_name}/llm_int8_01_Qwen-${model_size}-Chat
else
    ...
    save_result_path=/home/jinrenren/quant_eval_results/memory/${dataset_name}/llm_int8_01_Qwen-${model_size}-Chat
fi
...
if [[ ${#device} -eq 0 ]]; then
    python -u ../evaluate_llm_int8_quant_qwen_chat.py \
        ...
        --save_result_path ${save_result_path} > logs/evaluate_llm_int8_quant_qwen_chat.${dataset_name}.llm_int8_01_Qwen-${model_size}-Chat.logs 2>&1
else
    CUDA_VISIBLE_DEVICES=${device} python -u ../evaluate_llm_int8_quant_qwen_chat.py \
        --checkpoint_path ${checkpoint_path} \
        --eval_data_path ${eval_data_path} \
        --filed_name ${filed_name} \
        --batch_size ${batch_size} \
