        ...
        **kwargs,
        ...
        for block_start in block_start_iter:
            ...
            for column_index in range(block_start, block_end):
                if column_index % groupsize == 0:
                    ...
                    if save_quantization:
                        if quantizer.qq_scale_bits is not None:
                            save_quant_dict["quant_layer_scale"].append(quantizer.quant_scale.to(torch.uint8))
                        ...
                        if quantizer.qq_zero_bits is not None and (
                            ...
                            save_quant_dict["quant_layer_zeros"].append(quantizer.quant_zero.to(torch.uint8))
                ...
                if save_quantization:
                    save_quant_dict["quant_weights"].append(weight_quant_i.to(torch.uint8))
