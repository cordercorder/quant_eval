...
quantized_model_dir=${work_dir}/pretrained_models.quant/spqr_w${2}g16_without_outlier_2quant_Qwen-${model_size}-Chat

mkdir -p ${quantized_model_dir}
# 指定了alpaca_gpt4_data的话，CUSTOM_DATA_PATH就没用了


# 小的group_size + 二次量化 + no_outlier
CUDA_VISIBLE_DEVICES=${gpu} python main.py $MODEL_PATH ${CUSTOM_DATA_PATH} \
    --wbits $2 \
    --groupsize 16 \
    --perchannel \
    --qq_scale_bits 3 \
