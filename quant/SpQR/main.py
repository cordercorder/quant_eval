...
def get_inps(model, data_iterable, args, dev, nsamples=None):
    ...
    if emb_dev.type != "cuda":
        ...
        if model.config.model_type == "qwen":
            model.transformer.rotary_emb = model.transformer.rotary_emb.to(dev)
    ...
    if model.config.model_type == 'qwen':
        # head_mask 默认是None不用写
        # Qwen 最新的代码不要registered_causal_mask
        # forward_arg_names.append('registered_causal_mask')
        forward_arg_names.append('rotary_pos_emb_list')
    if model.config.model_type == 'chatglm':
        forward_arg_names.append('rotary_pos_emb')

    cache = {"i": 0, "attention_mask": [], "alibi": []}

    class Catcher(nn.Module):
        def __init__(self, module, model_type = None):
            super().__init__()
            self.module = module

            if model_type == 'chatglm':
                self.forward = self.chatGLM_forward

        def forward(self, inp, **kwargs):
            inps[cache["i"]] = inp.squeeze(0)
            cache["i"] += 1
            for forward_arg_name in forward_arg_names:
                if forward_arg_name in cache:
                    cache[forward_arg_name].append(kwargs.get(forward_arg_name))
                else:
                    cache[forward_arg_name] = [kwargs.get(forward_arg_name)]
            raise ValueError
        def chatGLM_forward(self, inp, attention_mask, rotary_pos_emb, **kwargs):
            # chatglm embedding后维度是 [S, B, H] [S, 1, H]
            # 正常是 [B, S, H] [1, S, H]
            # 并且ChatGLM用的是position args而不是kwargs
            inps[cache["i"]] = inp.squeeze(1) # ChatGLM的batch size维度在中间
            cache["i"] += 1
            cache["attention_mask"] = attention_mask
            cache['rotary_pos_emb'] = rotary_pos_emb
            raise ValueError

    layers[0] = Catcher(layers[0], model.config.model_type)
    ...
    if model.config.model_type == "qwen":
        model.transformer.rotary_emb = model.transformer.rotary_emb.to(emb_dev)
    if model.config.model_type == "chatglm":
        model.transformer.rotary_pos_emb = model.transformer.rotary_pos_emb.to(dev)
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for j in trange(args.nsamples, desc="calc outs before quantization", leave=False):
                sample_forward_args = {
                    k: v[j] for k, v in forward_args.items()
                }
                for k, v in sample_forward_args.items():
                    sample_forward_args[k] = v.to(layer_dev) if isinstance(v, torch.Tensor) else v
                if model_type == 'chatglm':
                    outs[j] = layer(inps[j].to(layer_dev).unsqueeze(1), **sample_forward_args)[0].squeeze(1)
                else:
                    outs[j] = layer(inps[j].to(layer_dev).unsqueeze(0), **sample_forward_args)[0].squeeze(0)
        ...
        for j in trange(args.nsamples, desc="calc outs after quantization", leave=False):
            ...
            for k, v in sample_forward_args.items():
                sample_forward_args[k] = v.to(layer_dev) if isinstance(v, torch.Tensor) else v
            if model_type == 'chatglm':
                outs_batch = layer(inps[j].to(layer_dev).unsqueeze(1), **sample_forward_args)[0].squeeze(1)
            else:
                outs_batch = layer(inps[j].to(layer_dev).unsqueeze(0), **sample_forward_args)[0].squeeze(0)
...
def perplexity_eval(model, testenc, args, dev):
    ...
    for i in trange(len(layers), desc="processing eval data by layer"):
        ...
        for j in range(nsamples):
            if model_type == 'chatglm':
                outs[j] = layer(inps[j].to(dev).unsqueeze(1), **forward_args)[0].squeeze(1)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--do_eval",
        action="store_true",
        help="if do eval after quantization.",
    )
    parser.add_argument(
        "--seqlen",
        type=int,
        default=2048,
        help="Quantize dataset/model seqlen",
    )
    ...
    print("============  Loading model... ============")
    model = get_model(args.model_path, args.seqlen, args.load, args.dtype).train(False)
