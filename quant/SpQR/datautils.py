...
def get_alpaca_gpt4(nsamples, seqlen, tokenizer, eval_mode=False, file_path = './data/alpaca_gpt4_data.json'):
    if eval_mode:
        raise Exception(
            "alpaca_gpt4 can not implented to eval_mode"
        )
    with open(file_path) as f:
        data = json.load(f)
    trainloader = []

    sample_idx = np.random.choice(len(data), nsamples)
    samples = []
    ...
    input_ids_list = preprocess(samples, tokenizer)
    
    for input_ids in input_ids_list:
        trainloader.append((input_ids, None))
    return trainloader


def get_loaders(name: str, nsamples=128, seed=0, seqlen=2048, eval_mode=False, model_path=None):
    """
    ...
    if name.lower() == "pajama":
        ...
        return None
    elif os.path.isfile(name) and not name.endswith("alpaca_gpt4_data.json"):
        try:
        ...
        elif name.lower() == "alpaca_gpt4_data":
            data = get_alpaca_gpt4(nsamples, seqlen, tokenizer, eval_mode=eval_mode)
        elif name.endswith("alpaca_gpt4_data.json"):
            data = get_alpaca_gpt4(nsamples, seqlen, tokenizer, eval_mode=eval_mode, file_path = name)
        else:
            raise ValueError(
